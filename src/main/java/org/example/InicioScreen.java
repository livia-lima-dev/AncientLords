...
public class InicioScreen extends Screen {
    ...
    private void setupMouseInput() {
        Input.mouse().onClicked(event -> {
            ...
            if (perfilRect != null && perfilRect.contains(mousePosition)) {
                Game.screens().display("perfil");
                return;
            }

            if (configuracoesRect != null && configuracoesRect.contains(mousePosition)) {
                Game.screens().display("configuracoes");
                return;
            }

            if (sairRect != null && sairRect.contains(mousePosition)) {
                Game.exit();
            }
        });
    }
    ...
    private void calcularMedidas() {
        ...
        startY = ((screenHeight - ((buttonHeight * 5) + (spacing * 4))) / 2) + 100;
    }
    ...
    private void renderBotoes(Graphics2D g) {
        int iniciarY = startY;
        int continuarY = startY + (buttonHeight + spacing);
        int perfilY = startY + ((buttonHeight + spacing) * 2);
        int configuracoesY = startY + ((buttonHeight + spacing) * 3);
        int sairY = startY + ((buttonHeight + spacing) * 4);

        g.drawImage(iniciarButton, buttonX, iniciarY, buttonWidth, buttonHeight, null);
        g.drawImage(continuarButton, buttonX, continuarY, buttonWidth, buttonHeight, null);
        g.drawImage(perfilButton, buttonX, perfilY, buttonWidth, buttonHeight, null);
        g.drawImage(configuracoesButton, buttonX, configuracoesY, buttonWidth, buttonHeight, null);
        g.drawImage(sairButton, buttonX, sairY, buttonWidth, buttonHeight, null);

        perfilRect = new Rectangle(buttonX, perfilY, buttonWidth, buttonHeight);
        configuracoesRect = new Rectangle(buttonX, configuracoesY, buttonWidth, buttonHeight);
        sairRect = new Rectangle(buttonX, sairY, buttonWidth, buttonHeight);
    }
}